...
public:
   ...
   Gdevices() {
     ...
     if (DEBUG_IO) std::cout << "Into gpublasCreate" << std::endl;
     ...
     for (int i=0; i<2; ++i)
       NWPW_GPU_ERROR(gpuMalloc(reinterpret_cast<void **>(&d_info[i]), sizeof(int)));
   }
   ...
   ~Gdevices() noexcept(false) {
      ...
      {
         inuse[i] = false;
         NWPW_GPU_ERROR(cudaFree(dev_mem[i]));
      }
      ndev_mem = 0;
      ...
      {
         inuse_large[i] = false;
         NWPW_GPU_ERROR(cudaFree(dev_mem_large[i]));
      }
      ndev_mem_large = 0;
      ...
      for (auto i=0; i<2; ++i)
         NWPW_GPU_ERROR(cudaFree(d_info[i]));
   ...
   */
   void TN4_dgemm(int npack2, int ne, double alpha, double *host_a,
     ...
     if (std::fabs(beta) > 0.0) {
       NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic11], host_caa, ne * ne * sizeof(double), gpuMemcpyHostToDevice));
       NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic12], host_cab, ne * ne * sizeof(double), gpuMemcpyHostToDevice));
       NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic21], host_cba, ne * ne * sizeof(double), gpuMemcpyHostToDevice));
       NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic22], host_cbb, ne * ne * sizeof(double), gpuMemcpyHostToDevice));
     }

     // copy host_a,host_b --> dev_mem
     NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), &host_a[tile_start2[0]], npack2,
                                            dev_mem[ia_psi[0]], tile_npack2[0], stream[0]));
     NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), &host_b[tile_start2[0]], npack2,
     ...
     for (auto tt = 0; tt < tile_fac; ++tt) {
       int ttp1 = tt + 1;
       if (ttp1 < tile_fac) {
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], ne, sizeof(double), &host_a[tile_start2[ttp1]], npack2,
                                                dev_mem[ia_psi[ttp1 % 2]], tile_npack2[ttp1], stream[ttp1 % 2]));
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], ne, sizeof(double), &host_b[tile_start2[ttp1]], npack2,
                                                dev_mem[ia_hpsi[ttp1 % 2]], tile_npack2[ttp1], stream[ttp1 % 2]));
       }
       NWPW_GPU_ERROR(gpuStreamSynchronize(stream[tt % 2]));
       NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matT, matN, ne, ne, tile_npack2[tt], &alpha,
     ...
     NWPW_GPU_ERROR(gpuMemcpy(host_caa, dev_mem[ic11], ne * ne * sizeof(double), gpuMemcpyDeviceToHost));
     NWPW_GPU_ERROR(gpuMemcpy(host_cab, dev_mem[ic12], ne * ne * sizeof(double), gpuMemcpyDeviceToHost));
     NWPW_GPU_ERROR(gpuMemcpy(host_cba, dev_mem[ic21], ne * ne * sizeof(double), gpuMemcpyDeviceToHost));
     NWPW_GPU_ERROR(gpuMemcpy(host_cbb, dev_mem[ic22], ne * ne * sizeof(double), gpuMemcpyDeviceToHost));
   ...
   */
    ...
    **************************************/
    void TN_dgemm2c(int n, int m, int npack2, int nida2, double *host_a, double *host_b, double *host_c)
    {
       ...
       NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(npack2,n,sizeof(double),host_a,npack2,dev_mem[ia],npack2,stream[0]));
       NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(npack2,m,sizeof(double),host_b,npack2,dev_mem[ib],npack2,stream[0]));

       NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
       NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matT, matN, n, m, npack2, &rtwo,
                                     dev_mem[ia], npack2,
                                     dev_mem[ib], npack2,
                                     &rzero,
                                     dev_mem[ic],n));

       NWPW_GPU_ERROR(gpuMemcpy(host_c,dev_mem[ic],n*m*sizeof(double),gpuMemcpyDeviceToHost));
   ...
   void NN_dgemm(int npack2, int ne, double alpha, double *host_a, double *host_b, double beta, double *host_c)
   {
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, ne, sizeof(double), host_b, ne, dev_mem[ib], ne, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), host_a+tile_start2[0], npack2,
                                             dev_mem[ia_psi[0]], tile_npack2[0], stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), host_c+tile_start2[0], npack2,
                                             dev_mem[ia_hpsi[0]], tile_npack2[0], stream[0]));
      ...
      for (auto tt = 0; tt < tile_fac; ++tt)
      {
         ...
         if (ttp1 < tile_fac)
         {
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], ne, sizeof(double), host_a+tile_start2[ttp1], npack2,
                                                   dev_mem[ia_psi[ttp1 % 2]], tile_npack2[ttp1], stream[ttp1 % 2]));
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], ne, sizeof(double), host_c+tile_start2[ttp1], npack2,
                                                   dev_mem[ia_hpsi[ttp1 % 2]], tile_npack2[ttp1], stream[ttp1 % 2]));
         }
         NWPW_GPU_ERROR(gpuStreamSynchronize(stream[tt % 2]));
         NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matN, tile_npack2[tt], ne, ne, &alpha,
                                       dev_mem[ia_psi[tt % 2]],   tile_npack2[tt],
                                       dev_mem[ib], ne,
                                       &beta,
                                       dev_mem[ia_hpsi[tt % 2]], tile_npack2[tt]));
         NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(tile_npack2[tt], ne, sizeof(double), dev_mem[ia_hpsi[tt % 2]], tile_npack2[tt],
                  ...
                  double *host_c,int ldc)
   {
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(lda, k, sizeof(double), host_a, lda, dev_mem[ia], lda, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ldb, n, sizeof(double), host_b, ldb, dev_mem[ib], ldb, stream[0]));

      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle,matN,matN,m,n,k,
                                    &alpha,
                                    dev_mem[ia],lda,
                                    dev_mem[ib],ldb,
                                    &beta,
                                    dev_mem[ic],ldc));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(ldc,n,sizeof(double),dev_mem[ic],ldc,host_c,ldc,stream[0]));
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
                  ...
                  double *host_c,int ldc)
   {
      ...
      int ib = fetch_dev_mem_indx(((size_t)ldb) * ((size_t)n));
      int ic = fetch_dev_mem_indx(((size_t)ldc) * ((size_t)n));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(lda, m, sizeof(double), host_a, lda, dev_mem[ia], lda, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ldb, n, sizeof(double), host_b, ldb, dev_mem[ib], ldb, stream[0]));

      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matT, matN, m, n, k,
                                    &alpha,
                                    dev_mem[ia], lda,
                                    dev_mem[ib], ldb,
                                    &beta,
                  ...
                  double *host_c,int ldc)
   {
      ...
      int ic = fetch_dev_mem_indx(((size_t)ldc) * ((size_t)n));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(lda,k,sizeof(double),host_a,lda,dev_mem[ia],lda,stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ldb,k,sizeof(double),host_b,ldb,dev_mem[ib],ldb,stream[0]));

      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matT, m, n, k,
                                    &alpha,
                                    dev_mem[ia], lda,
                                    dev_mem[ib], ldb,
                                    &beta,
                 ...
                 double *host_b, double beta, double *host_c)
   {
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, nprj, sizeof(double), host_c, ne,
                                             dev_mem[ic], ne, stream[0]));

      if (tile_fac > 1)
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), a_psi+tile_start2[0], npack2,
                                                dev_mem[ia_psi[0]], tile_npack2[0], stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], nprj, sizeof(double), b_prj+tile_start2[0], npack2,
      ...
      for (auto tt = 0; tt < tile_fac; ++tt) {
         int ttp1 = tt + 1;
         if (ttp1 < tile_fac) {
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], ne, sizeof(double), a_psi+tile_start2[ttp1], npack2,
                                                   dev_mem[ia_psi[ttp1%2]], tile_npack2[ttp1], stream[ttp1 % 2]));
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], nprj, sizeof(double), b_prj+tile_start2[ttp1], npack2,
      ...
      gpuMemcpy(host_c, dev_mem[ic], ne * nprj * sizeof(double), gpuMemcpyDeviceToHost);
                 ...
                 double *host_b, double beta, double *host_c)
   {
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, nprj, sizeof(double), host_b, ne,
                                             dev_mem[ib], ne, stream[(tile_fac - 1) % 2]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[tile_fac - 1], ne, sizeof(double), &host_c[tile_start2[tile_fac - 1]], npack2,
                                             dev_mem[ia_hpsi[(tile_fac - 1) % 2]], tile_npack2[tile_fac - 1], stream[(tile_fac - 1) % 2]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[tile_fac - 1], nprj, sizeof(double), &host_a[tile_start2[tile_fac - 1]], npack2,
                                             dev_mem[ib_prj[(tile_fac - 1) % 2]], tile_npack2[tile_fac - 1], stream[(tile_fac - 1) % 2]));
      for (auto tt = tile_fac - 1; tt >= 0; --tt) {
         int ttm1 = tt - 1;
         if (ttm1 >= 0) {
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttm1], ne, sizeof(double), &host_c[tile_start2[ttm1]], npack2,
                                                   dev_mem[ia_hpsi[ttm1 % 2]], tile_npack2[ttm1], stream[ttm1 % 2]));
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttm1], nprj, sizeof(double), &host_a[tile_start2[ttm1]], npack2,
                  ...
                  double *host_sa0, double *host_sa1, double *host_st1)
   {
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, ne, sizeof(double), host_s12, ne, dev_mem[i_s12], ne, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, ne, sizeof(double), host_s11, ne, dev_mem[i_s11], ne, stream[1]));

      NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_s21, ne, dev_mem[i_s21], ne));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_sa0, ne, dev_mem[i_sa0], ne));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_s11, ne, dev_mem[i_s11], ne));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_sa1, ne, dev_mem[i_sa1], ne));

      // mmm_Multiply(ms, s21, sa0, 1.0, sa1, 1.0);
      NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matN, ne, ne, ne, &rone,
                                    dev_mem[i_s21], ne,
                                    dev_mem[i_sa0], ne,
                                    &rone,
                                    dev_mem[i_sa1], ne));
      ...
      NWPW_GPUBLAS_ERROR(gpublasGetMatrix(ne, ne, sizeof(double), dev_mem[i_sa1], ne, host_sa1, ne));
   ...
   void NN1_zgemm(int npack1_max, int npack, int ne, double *alpha, double *host_a, double *host_b,
      ...
      // Assuming fetch_dev_mem_indx, NWPW_GPUBLAS_ERROR, NWPW_GPU_ERROR, master_handle, matN,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_a, 2*npack1_max, dev_mem[ia], 2*npack1_max, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne,         ne, sizeof(double), host_b, 2*ne,         dev_mem[ib], 2*ne,         stream[0]));
      if (!isCmplxZero(beta))
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_c, 2*npack1_max, dev_mem[ic], 2*npack1_max, stream[0]));
   ...
   void CN1_zgemm(int npack1_max, int npack, int ne, double *alpha, double *host_a,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_a, 2*npack1_max, dev_mem[ia], 2*npack1_max, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_b, 2*npack1_max, dev_mem[ib], 2*npack1_max, stream[0]));
      if (!isCmplxZero(beta))
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_c, 2*ne, dev_mem[ic], 2*ne, stream[0]));
   ...
   void CN2_zgemm(int ne, int nprj, int npack, int npack1_max, double *alpha, double *host_a,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne,   sizeof(double), host_a, 2*npack1_max, dev_mem[ia], 2*npack1_max, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, nprj, sizeof(double), host_b, 2*npack1_max, dev_mem[ib], 2*npack1_max, stream[0]));
      if (!isCmplxZero(beta))
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, nprj, sizeof(double), host_c, 2*ne, dev_mem[ic], 2*ne, stream[0]));
   ...
   void CN2_stride_zgemm(int ne, int nprj, int npack, int npack1_max, double *alpha, double *host_a,
      ...
      if (tile_fac > 1)
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), a_psi+tile_start2[0], 2*npack1_max, dev_mem[ia_psi[0]], tile_npack2[0], stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0],  nprj, sizeof(double), b_prj+tile_start2[0], 2*npack1_max, dev_mem[ib_prj[0]], tile_npack2[0], stream[0]));
      ...
      for (auto tt = 0; tt < tile_fac; ++tt)
       {
         ...
         if (ttp1 < tile_fac)
         {
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], ne, sizeof(double), a_psi+tile_start2[ttp1], 2*npack1_max,
                                                   dev_mem[ia_psi[ttp1%2]], tile_npack2[ttp1], stream[ttp1 % 2]));
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttp1], nprj, sizeof(double), b_prj+tile_start2[ttp1], 2*npack1_max,
                                                   dev_mem[ib_prj[ttp1%2]], tile_npack2[ttp1], stream[ttp1 % 2]));
         }
         NWPW_GPU_ERROR(gpuStreamSynchronize(stream[tt % 2]));
      ...
      gpuMemcpy(host_c, dev_mem[ic], 2*ne*nprj*sizeof(double), gpuMemcpyDeviceToHost);
   ...
   void NC2_zgemm(int npack1_max, int npack, int ne, int nprj,  double *alpha, double *host_a,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max,  nprj, sizeof(double), host_a, 2*npack1_max, dev_mem[ia], 2*npack1_max, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne,          nprj, sizeof(double), host_b, 2*ne,         dev_mem[ib], 2*ne,         stream[0]));
      if (!isCmplxZero(beta))
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_c, 2*npack1_max, dev_mem[ic], 2*npack1_max, stream[0]));
   ...
   void NC2_stride_zgemm(int npack1_max, int npack, int ne, int nprj,  double *alpha, double *host_a,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, nprj, sizeof(double), host_b, 2*ne, dev_mem[ib], 2*ne, stream[(tile_fac-1) % 2]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[tile_fac-1], ne, sizeof(double), host_c + tile_start2[tile_fac-1], 2*npack1_max,
                                             dev_mem[ia_hpsi[(tile_fac-1) % 2]], tile_npack2[tile_fac-1], stream[(tile_fac - 1) % 2]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[tile_fac-1], nprj, sizeof(double), host_a+tile_start2[tile_fac-1], 2*npack1_max,
                                             dev_mem[ib_prj[(tile_fac-1) % 2]], tile_npack2[tile_fac-1], stream[(tile_fac-1) % 2]));
      for (auto tt = tile_fac - 1; tt >= 0; --tt)
      {
         int ttm1 = tt - 1;
         if (ttm1 >= 0) {
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttm1], ne, sizeof(double), host_c+tile_start2[ttm1], 2*npack1_max,
                                                   dev_mem[ia_hpsi[ttm1%2]], tile_npack2[ttm1], stream[ttm1 % 2]));
            NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[ttm1], nprj, sizeof(double), host_a+tile_start2[ttm1], 2*npack1_max,
                                                   dev_mem[ib_prj[ttm1%2]], tile_npack2[ttm1], stream[ttm1 % 2]));
         }
         NWPW_GPU_ERROR(gpuStreamSynchronize(stream[tt%2]));
   ...
   void NN_zgemm(int m, int n, int k,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*lda, k, sizeof(double), host_a, 2*lda, dev_mem[ia], 2*lda, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ldb, n, sizeof(double), host_b, 2*ldb, dev_mem[ib], 2*ldb, stream[0]));
      if (!isCmplxZero(beta))
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ldc, n, sizeof(double), host_c, 2*ldc, dev_mem[ic], 2*ldc, stream[0]));
   ...
   void CN_zgemm(int m, int n, int k,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*lda, m, sizeof(double), host_a, 2*lda, dev_mem[ia], 2*lda, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ldb, n, sizeof(double), host_b, 2*ldb, dev_mem[ib], 2*ldb, stream[0]));
      if (!isCmplxZero(beta))
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ldc, n, sizeof(double), host_c, 2*ldc, dev_mem[ic], 2*ldc, stream[0]));
   ...
   void CN3_zgemm(int npack1_max, int npack, int ne, double *alpha, double *host_a,
      ...
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_a, 2*npack1_max, dev_mem[ia], 2*npack1_max, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*npack1_max, ne, sizeof(double), host_b, 2*npack1_max, dev_mem[ib], 2*npack1_max, stream[1]));
      if (!isCmplxZero(beta))
      {
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_caa, 2*ne, dev_mem[icaa], 2*ne, stream[0]));
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_cab, 2*ne, dev_mem[icab], 2*ne, stream[1]));
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_cbb, 2*ne, dev_mem[icbb], 2*ne, stream[1]));
      }
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      ...
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2*ne,ne,sizeof(double),dev_mem[icaa],2*ne,host_caa,2*ne,stream[0]));
      ...
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2*ne,ne,sizeof(double),dev_mem[icab],2*ne,host_cab,2*ne,stream[1]));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2*ne,ne,sizeof(double),dev_mem[icbb],2*ne,host_cbb,2*ne,stream[1]));
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[1]));
      inuse[ia] = false;
      inuse[ib] = false;
      inuse[icaa] = false;
   ...
   void CN4_zgemm(int npack1_max, int npack, int ne, double *alpha, double *host_a,
      ...
      if (!isCmplxZero(beta))
      {
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_caa, 2*ne, dev_mem[icaa], 2*ne, stream[0]));
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_cab, 2*ne, dev_mem[icab], 2*ne, stream[1]));
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_cba, 2*ne, dev_mem[icba], 2*ne, stream[1]));
         NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_cbb, 2*ne, dev_mem[icbb], 2*ne, stream[1]));
      }
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      ...
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2*ne,ne,sizeof(double),dev_mem[icab],2*ne,host_cab,2*ne,stream[1]));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2*ne,ne,sizeof(double),dev_mem[icba],2*ne,host_cba,2*ne,stream[1]));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2*ne,ne,sizeof(double),dev_mem[icbb],2*ne,host_cbb,2*ne,stream[1]));
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[1]));
...
*/
  ...
  void WW6_zgemm(int ne, double *host_s21, double *host_s12, double *host_s11,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_s21, 2*ne, dev_mem[is21], 2*ne, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_sa0, 2*ne, dev_mem[isa0], 2*ne, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_sa1, 2*ne, dev_mem[isa1], 2*ne, stream[0]));

    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_s12, 2*ne, dev_mem[is12], 2*ne, stream[1]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2*ne, ne, sizeof(double), host_s11, 2*ne, dev_mem[is11], 2*ne, stream[2]));
   ...
   void psi_copy_host2gpu(int npack1, int ne, double *psi)
   {
      ...
      a_psi = psi;
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[0], ne, sizeof(double), psi, 2*npack1,
                                             dev_mem[ia_psi[0]], tile_npack2[0], stream[0]));
   }
   ...
   void hpsi_copy_host2gpu(int npack1, int ne, double *hpsi)
   {
      ...
      int tt = tile_fac - 1;
      a_hpsi = hpsi;
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(tile_npack2[tt], ne, sizeof(double), &hpsi[tile_start2[tt]], 2 * npack1,
   ...
   void psi_copy_gpu2host(int npack1, int ne, double *psi)
   {
      ...
      {
         NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
         NWPW_GPUBLAS_ERROR(gpublasGetMatrix(tile_npack2[0], ne, sizeof(double), dev_mem[ia_psi[0]], tile_npack2[0],
                                           psi, 2*npack1));
      }
   }
   ...
   int batch_fft_init(int nx, int ny, int nz, int nq1, int nq2, int nq3)
   {
      ...
      if (NWPW_CUDA) {
        ...
        NWPW_ROCFFT_ERROR(rocfft_plan_create(&forward_plan_rx[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_real_forward, rocfft_precision_double, (size_t)1,
                                             &length_nx, (size_t)nq1, nullptr));
        NWPW_ROCFFT_ERROR(rocfft_plan_create(&backward_plan_rx[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_real_inverse, rocfft_precision_double, (size_t)1,
                                             &length_nx, (size_t)nq1, nullptr));

        NWPW_ROCFFT_ERROR(rocfft_plan_create(&forward_plan_x[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_complex_forward, rocfft_precision_double,
                                             (size_t)1, &length_nx, (size_t)nq1, nullptr));
        NWPW_ROCFFT_ERROR(rocfft_plan_create(&backward_plan_x[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_complex_inverse, rocfft_precision_double,
                                             (size_t)1, &length_nx, (size_t)nq1, nullptr));

        NWPW_ROCFFT_ERROR(rocfft_plan_create(&forward_plan_y[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_complex_forward, rocfft_precision_double,
                                             (size_t)1, &length_ny, (size_t)nq2, nullptr));
        NWPW_ROCFFT_ERROR(rocfft_plan_create(&backward_plan_y[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_complex_inverse, rocfft_precision_double,
                                             (size_t)1, &length_ny, (size_t)nq2, nullptr));

        NWPW_ROCFFT_ERROR(rocfft_plan_create(&forward_plan_z[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_complex_forward, rocfft_precision_double,
                                             (size_t)1, &length_nz, (size_t)nq3, nullptr));
        NWPW_ROCFFT_ERROR(rocfft_plan_create(&backward_plan_z[fftcount], rocfft_placement_inplace,
                                             rocfft_transform_type_complex_inverse, rocfft_precision_double,
                                             (size_t)1, &length_nz, (size_t)nq3, nullptr));
      } else if(NWPW_SYCL) {
        desc_r_x[fftcount] = new desc_real_t(nx);
        desc_r_x[fftcount]->set_value(oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, nq1);
        ...
        desc_z[fftcount]->set_value(oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, nq3);
  ...
  void batch_rfftx(const int fft_indx, bool forward, int nx, int nq, int n2ft3d, double *a)
  {
   ...
   void batch_rfftx_stages(const int stage, const int fft_indx, bool forward, int nx, int nq, int n2ft3d, double *a, int da)
   {
      ...
      {
         inuse[ia_dev] = true;
         NWPW_GPU_ERROR(gpuMemcpyAsync(dev_mem[ia_dev],a,n2ft3d*sizeof(double),gpuMemcpyHostToDevice,stream[da]));
      }
      else if (stage==1)
      {
