...
class Gdevices {
...
#ifdef NWPW_CUDA
  cufftHandle forward_plan_x[2] = {0, 0}, plan_x[2] = {0, 0},
              plan_y[2] = {0, 0}, plan_z[2] = {0, 0};
  ...
  gpublasHandle_t master_handle = 0;
  gpublasOperation_t matC = GPUBLAS_OP_C;
...
public:
  ...
  Gdevices() {
    ...
    if (DEBUG_IO)
      std::cout << "Into gpublasCreate" << std::endl;

    NWPW_GPUBLAS_ERROR(gpublasCreate(&master_handle));

    // allocate cuda/hip/sycl streams
    for (auto i = 0; i < 12; ++i)
      NWPW_GPU_ERROR(gpuStreamCreate(&stream[i]));

    // // create gpusolver handle, bind a stream
    // gpusolverDnCreate(&gpusolverH);
    // NWPW_GPU_ERROR(gpuStreamCreateWithFlags(&solverstream,
    // gpuStreamNonBlocking)); gpusolverDnSetStream(gpusolverH, solverstream);

    // query working space of syevd
    for (int i = 0; i < 2; ++i)
      NWPW_GPU_ERROR(
          gpuMalloc(reinterpret_cast<void **>(&d_info[i]), sizeof(int)));
  }
  ...
  ~Gdevices() noexcept(false) {
    ...
    for (auto i = 0; i < ndev_mem; ++i) {
      inuse[i] = false;
      NWPW_GPU_ERROR(gpuFree(dev_mem[i]));
    }
    ndev_mem = 0;
    ...
    for (auto i = 0; i < ndev_mem_large; ++i) {
      inuse_large[i] = false;
      NWPW_GPU_ERROR(gpuFree(dev_mem_large[i]));
    }
    ndev_mem_large = 0;

    // free cuda streams
    for (auto i = 0; i < 12; ++i)
      NWPW_GPU_ERROR(gpuStreamDestroy(stream[i]));

    NWPW_GPUBLAS_ERROR(gpublasDestroy(master_handle));

    /* free cusolver resources */
    for (auto i = 0; i < 2; ++i)
      NWPW_GPU_ERROR(gpuFree(d_info[i]));
    NWPW_GPU_ERROR(gpuFree(d_work));
    // CUSOLVER_CHECK(cusolverDnDestroy(cusolverH));

    // NWPW_GPU_ERROR(gpuStreamDestroy(solverstream));
    // cudaDeviceReset();
  ...
  int fetch_dev_mem_indx(const size_t ndsize) {
    ...
    if (ii < ndev_mem) {
      ...
      NWPW_GPU_ERROR(
          gpuMalloc((void **)&(dev_mem[ii]), ndsize * sizeof(double)));
      ndev_mem += 1;
      if (ndev_mem > NDEV_MAX)
        std::cout << "ERROR: ndev_mem > NDEV_MAX" << std::endl;
    }

    NWPW_GPU_ERROR(gpuMemset(dev_mem[ii], 0, ndsize * sizeof(double)));
    return ii;
  }
  ...
  int fetch_dev_mem_indx_large(const size_t ndsize0, const size_t ndsize1) {
    ...
    int ii = 0;
    while ((((ndsize != ndsize_mem_large[ii]) || inuse_large[ii])) &&
           (ii < ndev_mem_large))
      ++ii;
    ...
    if (ii < ndev_mem_large) {
      ...
      NWPW_GPU_ERROR(
          gpuMalloc((void **)&(dev_mem_large[ii]), ndsize * sizeof(double)));
      ndev_mem_large += 1;
      if (ndev_mem > NDEV_MAX_LARGE)
        std::cout << "ERROR: ndev_mem > NDEV_MAX_LARGE" << std::endl;
    }

    NWPW_GPU_ERROR(gpuMemset(dev_mem_large[ii], 0, ndsize * sizeof(double)));
    return ii;
  }
  ...
  */
  void TN4_dgemm(int npack2, int ne, double alpha, double *host_a,
    ...
    if (std::fabs(beta) > 0.0) {
      NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic11], host_caa,
                               ne * ne * sizeof(double),
                               gpuMemcpyHostToDevice));
      NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic12], host_cab,
                               ne * ne * sizeof(double),
                               gpuMemcpyHostToDevice));
      NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic21], host_cba,
                               ne * ne * sizeof(double),
                               gpuMemcpyHostToDevice));
      NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ic22], host_cbb,
                               ne * ne * sizeof(double),
                               gpuMemcpyHostToDevice));
    }

    // copy host_a,host_b --> dev_mem
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
    ...
    for (auto tt = 0; tt < tile_fac; ++tt) {
      ...
      NWPW_GPUBLAS_ERROR(gpublasDgemm(
    ...
    NWPW_GPU_ERROR(gpuMemcpy(host_caa, dev_mem[ic11], ne * ne * sizeof(double),
                             gpuMemcpyDeviceToHost));
    NWPW_GPU_ERROR(gpuMemcpy(host_cab, dev_mem[ic12], ne * ne * sizeof(double),
                             gpuMemcpyDeviceToHost));
    NWPW_GPU_ERROR(gpuMemcpy(host_cba, dev_mem[ic21], ne * ne * sizeof(double),
                             gpuMemcpyDeviceToHost));
    NWPW_GPU_ERROR(gpuMemcpy(host_cbb, dev_mem[ic22], ne * ne * sizeof(double),
                             gpuMemcpyDeviceToHost));
  ...
  */
   ...
   *                                    *
   **************************************/
  void TN_dgemm2c(int n, int m, int npack2, int nida2, double *host_a,
                  double *host_b, double *host_c) {
    double rtwo = 2.0;
    double rone = 1.0;
    double rmone = -1.0;
    double rzero = 0.0;
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(npack2, n, sizeof(double), host_a,
                                             npack2, dev_mem[ia], npack2,
                                             stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(npack2, m, sizeof(double), host_b,
                                             npack2, dev_mem[ib], npack2,
                                             stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matT, matN, n, m, npack2,
                                    &rtwo, dev_mem[ia], npack2, dev_mem[ib],
                                    npack2, &rzero, dev_mem[ic], n));

    NWPW_GPU_ERROR(gpuMemcpy(host_c, dev_mem[ic], n * m * sizeof(double),
                             gpuMemcpyDeviceToHost));

    if (nida2 > 0) {
      DGEMM_PWDFT((char *)"T", (char *)"N", n, m, nida2, rmone, host_a, npack2,
                  host_b, npack2, rzero, host_c, n);
    }
  ...
  void NN_dgemm(int npack2, int ne, double alpha, double *host_a,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, ne, sizeof(double), host_b, ne,
    ...
    for (auto tt = 0; tt < tile_fac; ++tt) {
      ...
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(
          tile_npack2[tt], ne, sizeof(double), dev_mem[ia_hpsi[tt % 2]],
          tile_npack2[tt], host_c + tile_start2[tt], npack2, stream[tt % 2]));
    }
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[(tile_fac - 1) % 2]));
    // NWPW_GPU_ERROR(
    // gpuMemcpy(host_c,dev_mem[ia_hpsi[0]],npack2*ne*sizeof(double),gpuMemcpyDeviceToHost)
    // );
   ...
   *                                    *
   **************************************/
  void NN_dgemm1(int m, int n, int k, double alpha, double *host_a, int lda,
                 double *host_b, int ldb, double beta, double *host_c,
                 int ldc) {
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(lda, k, sizeof(double), host_a,
                                             lda, dev_mem[ia], lda, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ldb, n, sizeof(double), host_b,
                                             ldb, dev_mem[ib], ldb, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matN, m, n, k, &alpha,
                                    dev_mem[ia], lda, dev_mem[ib], ldb, &beta,
                                    dev_mem[ic], ldc));
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(
        ldc, n, sizeof(double), dev_mem[ic], ldc, host_c, ldc, stream[0]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
   ...
   *                                    *
   **************************************/
  void TN_dgemm2(int m, int n, int k, double alpha, double *host_a, int lda,
                 double *host_b, int ldb, double beta, double *host_c,
                 int ldc) {
    int ia = fetch_dev_mem_indx(((size_t)lda) * ((size_t)m));
    int ib = fetch_dev_mem_indx(((size_t)ldb) * ((size_t)n));
    int ic = fetch_dev_mem_indx(((size_t)ldc) * ((size_t)n));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(lda, m, sizeof(double), host_a,
                                             lda, dev_mem[ia], lda, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ldb, n, sizeof(double), host_b,
                                             ldb, dev_mem[ib], ldb, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matT, matN, m, n, k, &alpha,
                                    dev_mem[ia], lda, dev_mem[ib], ldb, &beta,
                                    dev_mem[ic], ldc));
   ...
   *                                    *
   **************************************/
  void NT_dgemm3(int m, int n, int k, double alpha, double *host_a, int lda,
                 double *host_b, int ldb, double beta, double *host_c,
                 int ldc) {
    int ia = fetch_dev_mem_indx(((size_t)lda) * ((size_t)k));
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(lda, k, sizeof(double), host_a,
                                             lda, dev_mem[ia], lda, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ldb, k, sizeof(double), host_b,
                                             ldb, dev_mem[ib], ldb, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matT, m, n, k, &alpha,
                                    dev_mem[ia], lda, dev_mem[ib], ldb, &beta,
                                    dev_mem[ic], ldc));
  ...
  void TN_dgemm(int ne, int nprj, int npack2, double alpha, double *host_a,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, nprj, sizeof(double), host_c,
                                             ne, dev_mem[ic], ne, stream[0]));
    ...
    gpuMemcpy(host_c, dev_mem[ic], ne * nprj * sizeof(double),
              gpuMemcpyDeviceToHost);
  ...
  void NT_dgemm(int npack2, int ne, int nprj, double alpha, double *host_a,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(ne, nprj, sizeof(double), host_b,
                                             ne, dev_mem[ib], ne,
                                             stream[(tile_fac - 1) % 2]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
  ...
  void MM6_dgemm(int ne, double *host_s21, double *host_s12, double *host_s11,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_s21, ne,
                                        dev_mem[i_s21], ne));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_sa0, ne,
                                        dev_mem[i_sa0], ne));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_s11, ne,
                                        dev_mem[i_s11], ne));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrix(ne, ne, sizeof(double), host_sa1, ne,
                                        dev_mem[i_sa1], ne));

    // mmm_Multiply(ms, s21, sa0, 1.0, sa1, 1.0);
    NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matN, ne, ne, ne,
    ...
    NWPW_GPUBLAS_ERROR(gpublasGetMatrix(ne, ne, sizeof(double), dev_mem[i_sa1],
                                        ne, host_sa1, ne));
   ...
   *                                    *
   **************************************/
  void NN1_zgemm(int npack1_max, int npack, int ne, double *alpha,
                 double *host_a, double *host_b, double *beta, double *host_c) {
    // Assuming fetch_dev_mem_indx, NWPW_GPUBLAS_ERROR, NWPW_GPU_ERROR,
    // master_handle, matN, dev_mem, stream, and inuse are properly defined and
    // initialized elsewhere. ZGEMM_PWDFT((char *)"N", (char *)"N", npack, ne,
    // ne, alpha, host_a, npack1, host_b, ne, beta, host_c, npack1);
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        2 * npack1_max, ne, sizeof(double), host_a, 2 * npack1_max, dev_mem[ia],
        2 * npack1_max, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double), host_b,
                                             2 * ne, dev_mem[ib], 2 * ne,
                                             stream[0]));
    if (!isCmplxZero(beta))
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
          2 * npack1_max, ne, sizeof(double), host_c, 2 * npack1_max,
          dev_mem[ic], 2 * npack1_max, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasZgemm(
        master_handle, matN, matN, npack, ne, ne,
    ...
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(
        2 * npack1_max, ne, sizeof(double), dev_mem[ic], 2 * npack1_max, host_c,
        2 * npack1_max, stream[0]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
   ...
   *                                    *
   **************************************/
  void CN1_zgemm(int npack1_max, int npack, int ne, double *alpha,
                 double *host_a, double *host_b, double *beta, double *host_c) {
    // Assuming fetch_dev_mem_indx, NWPW_GPUBLAS_ERROR, NWPW_GPU_ERROR,
    // master_handle, matN, dev_mem, stream, and inuse are properly defined and
    // initialized elsewhere. ZGEMM_PWDFT((char *)"C", (char *)"N", ne, ne,
    // npack, alpha, host_a, npack1,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        ...
        2 * npack1_max, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        2 * npack1_max, ne, sizeof(double), host_b, 2 * npack1_max, dev_mem[ib],
        2 * npack1_max, stream[0]));
    if (!isCmplxZero(beta))
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                               host_c, 2 * ne, dev_mem[ic],
                                               2 * ne, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasZgemm(
        master_handle, matC, matN, ne, ne, npack,
        reinterpret_cast<const gpuDoubleComplex *>(alpha),
        reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia]), npack1_max,
        reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib]), npack1_max,
        reinterpret_cast<const gpuDoubleComplex *>(beta),
        reinterpret_cast<gpuDoubleComplex *>(dev_mem[ic]), ne));
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, ne, sizeof(double),
                                             dev_mem[ic], 2 * ne, host_c,
                                             2 * ne, stream[0]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
   ...
   *                                    *
   **************************************/
  void CN2_zgemm(int ne, int nprj, int npack, int npack1_max, double *alpha,
                 double *host_a, double *host_b, double *beta, double *host_c) {
    // Assuming fetch_dev_mem_indx, NWPW_GPUBLAS_ERROR, NWPW_GPU_ERROR,
    // master_handle, matN, dev_mem, stream, and inuse are properly defined and
    // initialized elsewhere.
    //  ZGEMM_PWDFT((char *)"C", (char *)"N", ne, nprj, npack, alpha, host_a,
    //  npack1,
    //              host_b, npack1, beta, host_c, ne);
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        ...
        2 * npack1_max, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        2 * npack1_max, nprj, sizeof(double), host_b, 2 * npack1_max,
        dev_mem[ib], 2 * npack1_max, stream[0]));
    if (!isCmplxZero(beta))
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, nprj, sizeof(double),
                                               host_c, 2 * ne, dev_mem[ic],
                                               2 * ne, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasZgemm(
        master_handle, matC, matN, ne, nprj, npack,
        reinterpret_cast<const gpuDoubleComplex *>(alpha),
        reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia]), npack1_max,
        reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib]), npack1_max,
        reinterpret_cast<const gpuDoubleComplex *>(beta),
        reinterpret_cast<gpuDoubleComplex *>(dev_mem[ic]), ne));
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, nprj, sizeof(double),
                                             dev_mem[ic], 2 * ne, host_c,
                                             2 * ne, stream[0]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
   ...
   *                                    *
   **************************************/
  void CN2_stride_zgemm(int ne, int nprj, int npack, int npack1_max,
                        double *alpha, double *host_a, double *host_b,
    ...
    if (tile_fac > 1)
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
          tile_npack2[0], ne, sizeof(double), a_psi + tile_start2[0],
          2 * npack1_max, dev_mem[ia_psi[0]], tile_npack2[0], stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        tile_npack2[0], nprj, sizeof(double), b_prj + tile_start2[0],
        2 * npack1_max, dev_mem[ib_prj[0]], tile_npack2[0], stream[0]));
    ...
    for (auto tt = 0; tt < tile_fac; ++tt) {
      ...
      if (ttp1 < tile_fac) {
        NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
            tile_npack2[ttp1], ne, sizeof(double), a_psi + tile_start2[ttp1],
            2 * npack1_max, dev_mem[ia_psi[ttp1 % 2]], tile_npack2[ttp1],
            stream[ttp1 % 2]));
        NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
            tile_npack2[ttp1], nprj, sizeof(double), b_prj + tile_start2[ttp1],
            2 * npack1_max, dev_mem[ib_prj[ttp1 % 2]], tile_npack2[ttp1],
            stream[ttp1 % 2]));
      }
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[tt % 2]));
      NWPW_GPUBLAS_ERROR(gpublasZgemm(
          master_handle, matC, matN, ne, nprj, tile_npack1[tt],
          reinterpret_cast<const gpuDoubleComplex *>(alpha),
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia_psi[tt % 2]]),
          tile_npack1[tt],
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib_prj[tt % 2]]),
          tile_npack1[tt], reinterpret_cast<const gpuDoubleComplex *>(beta0),
          reinterpret_cast<gpuDoubleComplex *>(dev_mem[ic]), ne));

      // NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matT, matN, ne, nprj,
      // tile_npack2[tt], &alpha,
    ...
    gpuMemcpy(host_c, dev_mem[ic], 2 * ne * nprj * sizeof(double),
              gpuMemcpyDeviceToHost);
   ...
   *                                    *
   **************************************/
  void NC2_zgemm(int npack1_max, int npack, int ne, int nprj, double *alpha,
                 double *host_a, double *host_b, double *beta, double *host_c) {
    // Assuming fetch_dev_mem_indx, NWPW_GPUBLAS_ERROR, NWPW_GPU_ERROR,
    // master_handle, matN, dev_mem, stream, and inuse are properly defined and
    // initialized elsewhere. ZGEMM_PWDFT((char *)"N", (char *)"C",
    // npack,ne,nprj, alpha, host_a, npack1,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        2 * npack1_max, nprj, sizeof(double), host_a, 2 * npack1_max,
        dev_mem[ia], 2 * npack1_max, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, nprj, sizeof(double),
                                             host_b, 2 * ne, dev_mem[ib],
                                             2 * ne, stream[0]));
    if (!isCmplxZero(beta))
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
          2 * npack1_max, ne, sizeof(double), host_c, 2 * npack1_max,
    ...
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasZgemm(
        master_handle, matN, matC, npack, ne, nprj,
        reinterpret_cast<const gpuDoubleComplex *>(alpha),
        reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia]), npack1_max,
        reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib]), ne,
        reinterpret_cast<const gpuDoubleComplex *>(beta),
   ...
   *                                    *
   **************************************/
  void NC2_stride_zgemm(int npack1_max, int npack, int ne, int nprj,
                        double *alpha, double *host_a, double *host_b,
                        double *beta, double *host_c) {
    // Assuming fetch_dev_mem_indx, NWPW_GPUBLAS_ERROR, NWPW_GPU_ERROR,
    // master_handle, matN, dev_mem, stream, and inuse are properly defined and
    ...
    NWPW_GPUBLAS_ERROR(
        gpublasSetMatrixAsync(2 * ne, nprj, sizeof(double), host_b, 2 * ne,
                              dev_mem[ib], 2 * ne, stream[(tile_fac - 1) % 2]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        tile_npack2[tile_fac - 1], ne, sizeof(double),
        host_c + tile_start2[tile_fac - 1], 2 * npack1_max,
        dev_mem[ia_hpsi[(tile_fac - 1) % 2]], tile_npack2[tile_fac - 1],
        stream[(tile_fac - 1) % 2]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        tile_npack2[tile_fac - 1], nprj, sizeof(double),
        host_a + tile_start2[tile_fac - 1], 2 * npack1_max,
        dev_mem[ib_prj[(tile_fac - 1) % 2]], tile_npack2[tile_fac - 1],
        stream[(tile_fac - 1) % 2]));
    for (auto tt = tile_fac - 1; tt >= 0; --tt) {
      int ttm1 = tt - 1;
      if (ttm1 >= 0) {
        NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
            tile_npack2[ttm1], ne, sizeof(double), host_c + tile_start2[ttm1],
            2 * npack1_max, dev_mem[ia_hpsi[ttm1 % 2]], tile_npack2[ttm1],
            stream[ttm1 % 2]));
        NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
            tile_npack2[ttm1], nprj, sizeof(double), host_a + tile_start2[ttm1],
            2 * npack1_max, dev_mem[ib_prj[ttm1 % 2]], tile_npack2[ttm1],
            stream[ttm1 % 2]));
      }
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[tt % 2]));

      NWPW_GPUBLAS_ERROR(gpublasZgemm(
          master_handle, matN, matC, tile_npack1[tt], ne, nprj,
          reinterpret_cast<const gpuDoubleComplex *>(alpha),
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib_prj[tt % 2]]),
          tile_npack1[tt],
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib]), ne,
          reinterpret_cast<const gpuDoubleComplex *>(beta),
          reinterpret_cast<gpuDoubleComplex *>(dev_mem[ia_hpsi[tt]]),
          tile_npack1[tt]));
      // NWPW_GPUBLAS_ERROR(gpublasDgemm(master_handle, matN, matT,
      // tile_npack2[tt],
      //                               ne, nprj, &alpha,
      //                               dev_mem[ib_prj[tt % 2]], tile_npack2[tt],
      //                               dev_mem[ib], ne,
      //                               &beta,
      //                               dev_mem[ia_hpsi[tt % 2]],
      //                               tile_npack2[tt]));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(
          tile_npack2[tt], ne, sizeof(double), dev_mem[ia_hpsi[tt % 2]],
          tile_npack2[tt], host_c + tile_start2[tt], 2 * npack1_max,
          stream[tt % 2]));
    }
   ...
   *                                    *
   **************************************/
  void NN_zgemm(int m, int n, int k, double *alpha, double *host_a, int lda,
                double *host_b, int ldb, double *beta, double *host_c,
                int ldc) {
    // ZGEMM_PWDFT((char *)"N", (char *)"N", m, n, k, alpha, host_a, lda,
    // host_b, ldb, beta, host_c, ldc);
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * lda, k, sizeof(double), host_a,
                                             2 * lda, dev_mem[ia], 2 * lda,
                                             stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ldb, n, sizeof(double), host_b,
                                             2 * ldb, dev_mem[ib], 2 * ldb,
                                             stream[0]));
    if (!isCmplxZero(beta))
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ldc, n, sizeof(double),
                                               host_c, 2 * ldc, dev_mem[ic],
                                               2 * ldc, stream[0]));

    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasZgemm(
        master_handle, matN, matN, m, n, k,
        reinterpret_cast<const gpuDoubleComplex *>(alpha),
    ...
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ldc, n, sizeof(double),
                                             dev_mem[ic], 2 * ldc, host_c,
                                             2 * ldc, stream[0]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
   ...
   *                                    *
   **************************************/
  void CN_zgemm(int m, int n, int k, double *alpha, double *host_a, int lda,
                double *host_b, int ldb, double *beta, double *host_c,
                int ldc) {
    // ZGEMM_PWDFT((char *)"C", (char *)"N", m, n, k, alpha, host_a, lda,
    // host_b, ldb, beta, host_c, ldc);
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * lda, m, sizeof(double), host_a,
                                             2 * lda, dev_mem[ia], 2 * lda,
                                             stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ldb, n, sizeof(double), host_b,
                                             2 * ldb, dev_mem[ib], 2 * ldb,
   ...
   *                                    *
   **************************************/
  void NC_zgemm(int m, int n, int k, double *alpha, double *host_a, int lda,
                double *host_b, int ldb, double *beta, double *host_c,
                int ldc) {
    // ZGEMM_PWDFT((char *)"N", (char *)"C", m, n, k, alpha, host_a, lda,
    // host_b, ldb, beta, host_c, ldc);
   ...
   *                                    *
   **************************************/
  void CN3_zgemm(int npack1_max, int npack, int ne, double *alpha,
                 double *host_a, double *host_b, double *beta, double *host_caa,
                 double *host_cab, double *host_cbb) {
    int one = 1;
    int shift1 = 0;
    int mshift1 = 0;
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
        2 * npack1_max, ne, sizeof(double), host_b, 2 * npack1_max, dev_mem[ib],
        2 * npack1_max, stream[1]));
    if (!isCmplxZero(beta)) {
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                               host_caa, 2 * ne, dev_mem[icaa],
                                               2 * ne, stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                               host_cab, 2 * ne, dev_mem[icab],
                                               2 * ne, stream[1]));
      NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                               host_cbb, 2 * ne, dev_mem[icbb],
                                               2 * ne, stream[1]));
    }
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    ...
    for (auto k = 1; k <= ne; ++k) {
      ...
      */
      NWPW_GPUBLAS_ERROR(gpublasZgemm(
          master_handle, matC, matN, k, one, npack,
          reinterpret_cast<const gpuDoubleComplex *>(alpha),
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia]), npack1_max,
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia] + shift1),
          npack1_max, reinterpret_cast<const gpuDoubleComplex *>(beta),
          reinterpret_cast<gpuDoubleComplex *>(dev_mem[icaa] + mshift1), k));
      shift1 += 2 * npack1_max;
      mshift1 += 2 * ne;
    }
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, ne, sizeof(double),
                                             dev_mem[icaa], 2 * ne, host_caa,
                                             2 * ne, stream[0]));
    ...
    for (auto k = 1; k <= ne; ++k) {
      NWPW_GPUBLAS_ERROR(gpublasZgemm(
          ...
          reinterpret_cast<const gpuDoubleComplex *>(alpha),
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ia]), npack1_max,
          reinterpret_cast<const gpuDoubleComplex *>(dev_mem[ib] + shift1),
          npack1_max, reinterpret_cast<const gpuDoubleComplex *>(beta),
    ...
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, ne, sizeof(double),
                                             dev_mem[icab], 2 * ne, host_cab,
                                             2 * ne, stream[1]));
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, ne, sizeof(double),
                                             dev_mem[icbb], 2 * ne, host_cbb,
                                             2 * ne, stream[1]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[1]));
    inuse[ia] = false;
    inuse[ib] = false;
   ...
   *                                    *
   **************************************/
  void CN4_zgemm(int npack1_max, int npack, int ne, double *alpha,
                 double *host_a, double *host_b, double *beta, double *host_caa,
                 double *host_cab, double *host_cba, double *host_cbb) {
    int one = 1;
    int shift1 = 0;
   ...
   **************************************/
  /**
   * @brief Computes the 3D transformation of projection data using
   * transformation matrices.
   *
   * This function computes transformation sums for multiple projection (`prj`)
   * functions and `psi` functions. It uses projection data (`prj`) and input
   * data (`psi`) to calculate intermediate values, and then computes the
   * transformation sums (`sum3`) using the transformation matrices (`Gx`, `Gy`,
   * `Gz`).
   *
   * @param ne    Number of `psi` functions to process.
   * @param nprj  Number of `prj` functions to process.
   * @param psi   Pointer to the input data array containing the `psi`
   * functions.
   * @param prj   Pointer to the projection data array containing the `prj`
   * functions.
   * @param ng    Grid size for the main computation (number of complex
   * numbers).
   * @param ng0   Reduced grid size for the secondary computation (number of
   * complex numbers).
   ...
   *
   * @note This function can be optimized using SIMD instructions, OpenMP for
   * parallelization, and GPU acceleration to enhance performance.
   */
  /*
  ...
  */
   ...
   **************************************/
  /**
   * @brief Computes the transformation of projection data using matrix
   * multiplication.
   *
   * This function computes matrix multiplication between projection (`prj`) and
   * input
   * (`psi`) data using a BLAS DGEMM routine, which is optimized for
   * double-precision general matrix multiplication. It first performs the
   * matrix multiplication with the full grid size (`npack2`), and if a reduced
   * grid size (`npack0`) is specified, subtracts the result using another
   * matrix multiplication.
   *
   * The BLAS DGEMM function can be optimized for various architectures,
   * leveraging specialized hardware and libraries to enhance performance.
   *
   * @param ne    Number of `psi` functions to process.
   * @param nprj  Number of `prj` functions to process.
   * @param psi   Pointer to the input data array containing the `psi`
   ...
   */
  /*
    void computeTrans_Mult(int ne, int nprj, double alpha, double alpha1, int
    ng, int ng0, double *psi, double *prj, double beta, double beta1, double
    *sum1)
     {
        int npack2 = 2*ng;
        int npack0 = 2*ng0;
        ...
        //DGEMM_PWDFT((char *)"T", (char
    *)"N",ne,nprj,npack2,alpha,psi,npack2,prj,npack2,beta,sum1,ne); if (npack0 >
    0)
        {
           DGEMM_PWDFT((char *)"T", (char
    *)"N",ne,nprj,npack0,alpha1,psi,npack2,prj,npack2,beta1,sum1,ne);
        }
     }
  */
  ...
  void WW6_zgemm(int ne, double *host_s21, double *host_s12, double *host_s11,
    ...
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                             host_s21, 2 * ne, dev_mem[is21],
                                             2 * ne, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                             host_sa0, 2 * ne, dev_mem[isa0],
                                             2 * ne, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                             host_sa1, 2 * ne, dev_mem[isa1],
                                             2 * ne, stream[0]));

    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                             host_s12, 2 * ne, dev_mem[is12],
                                             2 * ne, stream[1]));
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(2 * ne, ne, sizeof(double),
                                             host_s11, 2 * ne, dev_mem[is11],
                                             2 * ne, stream[2]));
    ...
    NWPW_GPUBLAS_ERROR(gpublasZgemm(
        master_handle, matN, matN, ne, ne, ne,
    ...
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, ne, sizeof(double),
                                             dev_mem[ist1], 2 * ne, host_st1,
                                             2 * ne, stream[0]));
    NWPW_GPUBLAS_ERROR(gpublasGetMatrixAsync(2 * ne, ne, sizeof(double),
                                             dev_mem[isa1], 2 * ne, host_sa1,
                                             2 * ne, stream[0]));
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
  ...
  void WW_eigensolver(int ispin, int ne[], double *host_hml, double *host_eig) {
    ...
    for (int ms = 0; ms < ispin; ++ms) {
      ...
      //  d3db::parall->Barrier();
      ZEIGEN_PWDFT(n, host_hml + shift2, host_eig + shift1, xmp1, nn, rmp1,
                   ierr);
  ...
  void psi_copy_host2gpu(int npack1, int ne, double *psi) {
    // gpuMemcpy(dev_mem[ia_psi[0]],psi,tile_npack2_max*ne*sizeof(double),gpuMemcpyHostToDevice);
    a_psi = psi;
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
  ...
  void hpsi_copy_host2gpu(int npack1, int ne, double *hpsi) {
    // gpuMemcpy(dev_mem[ia_hpsi[0]],hpsi,2*npack1*ne*sizeof(double),gpuMemcpyHostToDevice);
    int tt = tile_fac - 1;
    a_hpsi = hpsi;
    NWPW_GPUBLAS_ERROR(gpublasSetMatrixAsync(
  ...
  void psi_copy_gpu2host(int npack1, int ne, double *psi) {
    // gpuMemcpy(psi, dev_mem[ia_psi[0]],
    // 2*ne*npack1*sizeof(double),gpuMemcpyDeviceToHost);
    if (tile_fac == 1) {
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
      NWPW_GPUBLAS_ERROR(gpublasGetMatrix(tile_npack2[0], ne, sizeof(double),
  ...
  void hpsi_copy_gpu2host(int npack1, int ne, double *hpsi) {
    NWPW_GPU_ERROR(gpuStreamSynchronize(stream[0]));
    // gpuMemcpy(hpsi, dev_mem[ia_hpsi[0]],
    // 2*ne*npack1*sizeof(double),gpuMemcpyDeviceToHost); if (tile_fac==1) {
    //     NWPW_GPUBLAS_ERROR(
    //     gpublasGetMatrix(tile_npack2[0],ne,sizeof(double),
  ...
  int batch_fft_init(int nx, int ny, int nz, int nq1, int nq2, int nq3) {
...
#ifdef NWPW_CUDA
    NWPW_CUFFT_ERROR(
        cufftPlan1d(&forward_plan_x[fftcount], nx, CUFFT_D2Z, nq1));
    NWPW_CUFFT_ERROR(
        cufftPlan1d(&backward_plan_x[fftcount], nx, CUFFT_Z2D, nq1));

    int x_inembed[] = {nx};
    int x_onembed[] = {nx};
    NWPW_CUFFT_ERROR(cufftPlanMany(&plan_x[fftcount], 1, &nx, x_inembed, 1, nx,
                                   x_onembed, 1, nx, CUFFT_Z2Z, nq1));
    int y_inembed[] = {ny};
    int y_onembed[] = {ny};
    NWPW_CUFFT_ERROR(cufftPlanMany(&plan_y[fftcount], 1, &ny, y_inembed, 1, ny,
                                   y_onembed, 1, ny, CUFFT_Z2Z, nq2));
    int z_inembed[] = {nz};
    int z_onembed[] = {nz};
    NWPW_CUFFT_ERROR(cufftPlanMany(&plan_z[fftcount], 1, &nz, z_inembed, 1, nz,
    ...
    desc_r_x[fftcount]->set_value(
        oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, nq1);
    desc_r_x[fftcount]->set_value(oneapi::mkl::dft::config_param::FWD_DISTANCE,
                                  nx + 2);
    desc_r_x[fftcount]->set_value(oneapi::mkl::dft::config_param::BWD_DISTANCE,
                                  nx / 2 + 1);

    desc_x[fftcount] = new desc_cmplx_t(nx);
    desc_x[fftcount]->set_value(
        oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, nq2);
    desc_x[fftcount]->set_value(oneapi::mkl::dft::config_param::FWD_DISTANCE,
                                nx);
    desc_x[fftcount]->set_value(oneapi::mkl::dft::config_param::BWD_DISTANCE,
                                nx);

    desc_y[fftcount] = new desc_cmplx_t(ny);
    desc_y[fftcount]->set_value(
        oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, nq2);
    desc_y[fftcount]->set_value(oneapi::mkl::dft::config_param::FWD_DISTANCE,
                                ny);
    desc_y[fftcount]->set_value(oneapi::mkl::dft::config_param::BWD_DISTANCE,
                                ny);

    desc_z[fftcount] = new desc_cmplx_t(nz);
    desc_z[fftcount]->set_value(
        oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, nq3);
    desc_z[fftcount]->set_value(oneapi::mkl::dft::config_param::FWD_DISTANCE,
                                nz);
    desc_z[fftcount]->set_value(oneapi::mkl::dft::config_param::BWD_DISTANCE,
                                nz);
   ...
   *                                    *
   **************************************/
  void batch_rfftx(const int fft_indx, bool forward, int nx, int nq, int n2ft3d,
                   double *a) {
    int ia_dev = fetch_dev_mem_indx(((size_t)n2ft3d));
    NWPW_GPU_ERROR(gpuMemcpy(dev_mem[ia_dev], a, n2ft3d * sizeof(double),
                             gpuMemcpyHostToDevice));
    ...
    NWPW_GPU_ERROR(gpuMemcpy(a, dev_mem[ia_dev], n2ft3d * sizeof(double),
                             gpuMemcpyDeviceToHost));
  ...
  void batch_rfftx_stages(const int stage, const int fft_indx, bool forward,
    ...
    if (stage == 0) {
      inuse[ia_dev] = true;
      NWPW_GPU_ERROR(gpuMemcpyAsync(dev_mem[ia_dev], a, n2ft3d * sizeof(double),
                                    gpuMemcpyHostToDevice, stream[da]));
    } else if (stage == 1) {
      // NWPW_GPU_ERROR(gpuStreamSynchronize(stream[da]));
      if (forward) {
#ifdef NWPW_CUDA
        NWPW_GPUFFT_ERROR(cufftExecD2Z(
            forward_plan_x[fft_indx],
            reinterpret_cast<cufftDoubleReal *>(dev_mem[ia_dev]),
            reinterpret_cast<cufftDoubleComplex *>(dev_mem[ia_dev])));
#elif defined NWPW_HIP
        NWPW_GPUFFT_ERROR(rocfft_execute(
            forward_plan_rx[fft_indx],
            reinterpret_cast<void **>(&(dev_mem[ia_dev])), nullptr, nullptr));
#elif defined(NWPW_SYCL)
        compute_forward(*desc_x[fft_indx], dev_mem[ia_dev]);
...
#ifdef NWPW_CUDA
        NWPW_GPUFFT_ERROR(cufftExecZ2D(
      ...
      NWPW_GPU_ERROR(gpuMemcpyAsync(a, dev_mem[ia_dev], n2ft3d * sizeof(double),
                                    gpuMemcpyDeviceToHost, stream[da]));
    } else if (stage == 2) {
      NWPW_GPU_ERROR(gpuStreamSynchronize(stream[da]));
      inuse[ia_dev] = false;
    }
  ...
  void batch_cffty_stages(const int stage, const int fft_indx, bool forward,
                          int ny, int nq, int nffts, int n2ft3d, double *a, int da) {
    // int ia_dev = fetch_dev_mem_indx(((size_t)n2ft3d));
    int ia_dev = ifft_dev[da];
    if (stage == 0) {
      inuse[ia_dev] = true;
  ...
  void batch_cfftz(const int fft_indx, bool forward, int nz, int nq, int n2ft3d,
                   double *a) {
    int ia_dev = fetch_dev_mem_indx(((size_t)n2ft3d));
    gpuMemcpy(dev_mem[ia_dev], a, n2ft3d * sizeof(double),
              gpuMemcpyHostToDevice);

    if (forward) {
#ifdef NWPW_CUDA
      NWPW_CUFFT_ERROR(
          cufftExecZ2Z(plan_z[fft_indx],
                       reinterpret_cast<cufftDoubleComplex *>(dev_mem[ia_dev]),
                       reinterpret_cast<cufftDoubleComplex *>(dev_mem[ia_dev]),
                       CUFFT_FORWARD));
#elif defined(NWPW_HIP)
    ...
    gpuMemcpy(a, dev_mem[ia_dev], n2ft3d * sizeof(double),
              gpuMemcpyDeviceToHost);
  ...
  void batch_cfftz_stages(const int stage, const int fft_indx, bool forward,
                          int nz, int nq, int nffts, int n2ft3d, double *a, int da) {
    // int ia_dev = fetch_dev_mem_indx(((size_t)n2ft3d));
    int ia_dev = ifft_dev[da];
  ...
  /*
     ...
     *host_eig) { int i_a1[ispin],i_w1[ispin],info[ispin]; int shift1 = 0; int
     ...
     //input-output i_w1[ms] = fetch_dev_mem_indx(((size_t) ne[ms]) ); //output
           NWPW_GPU_ERROR(gpuMemcpyAsync(dev_mem[i_a1[ms]],host_hml+shift2,nn*sizeof(double),gpuMemcpyHostToDevice,stream[ms]));
           //NWPW_GPU_ERROR(gpuMemcpy(dev_mem[i_a1[ms]],host_hml+shift2,nn*sizeof(double),gpuMemcpyHostToDevice));
           //NWPW_GPU_ERROR(gpuMemcpyAsync(dev_mem[i_w1[ms]],host_eig+shift1,ne[ms]*sizeof(double),gpuMemcpyHostToDevice));
     ...
     **>(&d_work),sizeof(double) * lwork));
        ...
        {
           NWPW_GPU_ERROR( gpuStreamSynchronize(stream[ms]) );

           // compute spectrum
           int nn = ne[ms]*ne[ms];
           CUSOLVER_CHECK(cusolverDnDsyevd(cusolverH,jobz,uplo,ne[ms],dev_mem[i_a1[ms]],ne[ms],dev_mem[i_w1[ms]],d_work,lwork,d_info[ms]));

           NWPW_GPU_ERROR( gpuStreamSynchronize(solverstream) );

           NWPW_GPU_ERROR(gpuMemcpyAsync(host_hml+shift2,dev_mem[i_a1[ms]],nn*sizeof(double),gpuMemcpyDeviceToHost,stream[ms]));
           NWPW_GPU_ERROR(gpuMemcpyAsync(host_eig+shift1,dev_mem[i_w1[ms]],ne[ms]*sizeof(double),gpuMemcpyDeviceToHost,stream[ms]));
           NWPW_GPU_ERROR(gpuMemcpyAsync(info+ms,d_info[ms],sizeof(int),gpuMemcpyDeviceToHost,stream[ms]));
        ...
        {
           NWPW_GPU_ERROR(gpuStreamSynchronize(stream[ms]));
           NWPW_GPU_ERROR( cudaFree(d_work) );
  ...
  */
   ...
   *                                    *
   **************************************/
  inline static void fft_radix(const int n, const int s, bool eo,
                               const int radix, const complex_t *twiddle,
                               complex_t *x, complex_t *y) {
    if (n == 1) {
   ...
   *                                    *
   **************************************/
  inline static void fft_twiddle(const int n, const complex_t *twiddle,
                                 complex_t *x) // Fourier transform
  {
   ...
   *                                    *
   **************************************/
  static void set_sub_fft_twiddle(const int isgn, const int n, const int radix,
                                  complex_t *twiddle) {
   ...
   *                                    *
   **************************************/
  void batch_cfft(const bool forward, const int nz, const int nq,
                  const int nfft3d, double *a, const double *twiddle,
                  const double *tmpz) {
    ...
    const complex_t *complex_twiddle =
        reinterpret_cast<const complex_t *>(twiddle);
   ...
   *                                    *
   **************************************/
  void batch_cfft_zero(const bool forward, int nz, int nq, int nfft3d,
                       double *a, const double *twiddle, const double *tmpz,
                       const bool *zero) {
    // Ensure the function processes the right type of data
    // If twiddle is indeed of type complex_t, this cast is necessary
    complex_t *complex_a = reinterpret_cast<complex_t *>(a);
